...
data = spark.read.format("csv").option("header", True).option("inferSchema", True).option("delimiter", ",").load("C:/Users/Lalith Chandra A/Downloads/car.csv")
data = data.withColumn("label", when(col("engine-location") == "front", 1).otherwise(0)).select("label", "length")
